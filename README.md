...
With `listener.py` running, for parsing and visualizing data, a `filter` and `plotter` are necessary. In this repo, we provide specific examples `usbl_filter.py`, `sentry_filter.py`, and `sentry_dashboard.py` which can parse USBL mesages, and parse and plot messages from AUV Sentry. Both of the `_filter.py` utilities will read in a pointer to a raw UDP listener log file, and sort messages into new log files under different queue names (e.g., *science*, *status*, *ship*, *jason*, and so on). The USBL filter separates location information between different ID objects (like SENTRY, JASON, the ship, a CTD Rosette) and the SENTRY filter separates based on type of queue message. To run a filter, the following form can be used:
...
The new files created by the filter utilities can then be visualized by either a rudimentary `plotter` utility, or a more feature rich `dashboard` utility. We provide a dashboard utility in this repository; to run the dashboard, which provides a fully interactive set of html webpages on `127.0.0.1:8050`, the following command can be run:

```python sentry_dashboard.py -t ./data/sentry/proc_dive_{divenum}_science.txt -u ./data/usbl/proc_usbl_{divenum}.txt -b path/to/bathy/file```

This command points to the science data, usbl data, and a local bathy file. This is the minimum set of inputs needed to launch a dashboard, and will show simple time-based streaming and overhead mapping. Additional inputs that are supported include:
* the SAGE methane sensor (experimental sensor)
* a METS methane sensor
* an auxiliary high-sensitivity OBS sensor
* current data from external moorings (currently Ocean Networks Canada data is supported)
* a textfile of vent locations, for plotting
* a textfile of equipment (e.g., moorings) locations, for plotting
* specifying the exact names of data objects to render (if you would like anything beyond the default)
* specifying the subset of data objects you would like to plot (from the list of keys you provide)


Examples of how to run all utilities in a terminal are provided at the top of each script. Additionally, a convenience bash script, `run_watchstation.sh` has been provided which can be modified to run all the necessary commands to launch a full listening, filtering, and plotting mission.
...
For convenience, `run_spoofer.sh` has been provided which will set-up a spoofer instance and run all other listeners, filters, and plotters to test the dashboard services. It can be executed in a terminal with the command `./run_spoofer.sh`. 

## Running on a Mission
We suggest using `run_watchstation.sh` for running a real mission. Prior to launch, please do the following:
1. Open `port_config.yaml` and ensure that the port numbers (the SDQ numbers) and USBL target ID for the AUV are set correctly. You can get these numbers from the AUV operators or the marine techs onboard.
2. Open `run_watchstation.sh` and ensure that you have named your files for your dive, have the correct configuration of sensors selected, and have the desired plotting selections made. Follow the instructions within the file.
...
## Dependencies
These tools are written in Python 3.5+ and have been tested on Ubuntu 18.04, 20.04, and 22.04 operating systems. Python 3.10+ is recommended. 

An exhaustive list of requirements is noted in the `requirements.txt` file in the repo home directory, and can be used to install this repository directly.
...
Some science features, like "spice" and "potential density," are only approximations computed from the data that can be transmitted via the scalar science queue circa August 2023. These should not be interpreted as true values, however their overall patterns are realistic. If observations, such as CTD conductivity, were transmitted, these values could be directly computed live and the functions that compute these values can be updated to reflect these changes. 

## Adding New Queues and Filters
If you would like to add a new sensor to this project, you will need to modify `sentry_filter.py` and `filter_utils.py` to add a new message type and filter parsing to the project. If you would like to visualize the data from this sensor, you will additionally need to modify `sentry_dashboard.py` and `plotter_utils.py` to pass in the new data stream and render it in graphs. If you have trouble with making your own modification, please reach out to the project maintainer for assistance (vpreston).
